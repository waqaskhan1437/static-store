const fs = require('fs');
// generateProductHTML function taken from admin.js
  ...
  <div class="form-col">
    ...
    <div class="form-section">
      ...
      ${product.addons && product.addons.length > 0 ? `<div>${product.addonsHeadingText ? `<label style="color:${product.addonsHeadingColor || '#047857'};font-weight:700;display:block;margin-bottom:0.25rem;">${product.addonsHeadingText}</label>` : '<label>Add-ons</label>'}${product.addons.map((addon, idx) => {
        return `<label style="display:flex;justify-content:space-between;align-items:center;gap:0.5rem;margin-bottom:0.5rem;"><span style="display:flex;align-items:center;gap:8px;"><input type="checkbox" class="addon-checkbox" data-addon-price="${parseFloat(addon.price).toFixed(2)}"> ${addon.name} <input type="number" class="addon-qty" value="1" min="1" style="width:60px;margin-left:8px;"></span><span>${parseFloat(addon.price) > 0 ? '+$' + parseFloat(addon.price).toFixed(2) : 'Free'}</span></label>`;
      }).join('')}</div>` : ''}
...
function updatePrice(){
  ...
  let total = basePrice;
  // add selected delivery cost
  const selectedDelivery = document.querySelector('input[name="delivery"]:checked');
  if(selectedDelivery){
    const idx = parseInt(selectedDelivery.value);
    total += parseFloat(deliveryCosts[idx]) || 0;
  }
  // include photo option cost by parsing price from option text
  const photoSelect = document.getElementById('choose-photo-count');
  if(photoSelect){
    const selOpt = photoSelect.options[photoSelect.selectedIndex];
    if(selOpt){
      const txt = selOpt.textContent || '';
      const match = txt.match(/\$([0-9.]+)/);
      if(match){
        total += parseFloat(match[1]) || 0;
      } else if(/free/i.test(txt)) {
        total += 0;
      }
    }
  }
  // add cost of selected addons multiplied by quantity
  document.querySelectorAll('input[data-addon-price]')?.forEach(chk => {
    if(chk.checked){
      const price = parseFloat(chk.getAttribute('data-addon-price')) || 0;
      let qty = 1;
      const lbl = chk.closest('label');
      if(lbl){
        const qtyInput = lbl.querySelector('.addon-qty');
        if(qtyInput){
...
document.addEventListener('change', function(e){
  ...
  if(e.target && e.target.id === 'choose-photo-count'){
    ...
    // update price when photo selection changes
    updatePrice();
  }
...
// define products array same as patch
const products = [
  require('./tmp_products.js')[0], // placeholder, will replace later
];
